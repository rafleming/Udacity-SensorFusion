...
int main(int argc, const char *argv[])
{
    ...
    for (int testRun = 0; testRun < testRuns; ++testRun)
    {
        ...
        for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
        {
            ...
            if (circularDataBuffer.size() > 1)
            {
                ...
                matchDescriptors((circularDataBuffer.end() - 2)->keypoints, (circularDataBuffer.end() - 1)->keypoints,
    ...
    std::cout << "***** Matches: " << matchesCount/testRuns << endl;
    std::cout << "***** Time: " << (1000 * time)/(double)testRuns << " ms" << endl;
