...
struct KdTree
{
	...
	void insert(std::vector<float> point, int id)
	{
  	...
  	void findNearbyNodes(std::vector<float> target, Node* node, int depth, float distanceTol, std::vector<int>& ids)
    {
    	if (node != NULL)
        {
        	if ((node->point[0]>=(target[0]-distanceTol) && (node->point[0]<=(target[0]+distanceTol))) && 
          	...
          	if ((target[depth%2]-distanceTol)<node->point[depth%2])
            	findNearbyNodes(target, node->left, depth+1, distanceTol, ids);

          	if ((target[depth%2]+distanceTol)>node->point[depth%2])
            	findNearbyNodes(target, node->right, depth+1, distanceTol, ids);
        }
    }
	...
	{
		std::vector<int> ids;
      	findNearbyNodes(target, root, 0, distanceTol, ids);
		return ids;
	}
